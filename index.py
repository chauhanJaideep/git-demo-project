...
def Database():
    ...
    cursor.execute("SELECT * FROM `admin` WHERE `username` = 'darshal' AND `password` = 'darshal'")
    conn.commit()   
...
def Delete():
    if not tree.selection():
       tkMessageBox.showinfo("Warning","Please Select A Product From List")
    else:
...
def changePassword():
    ...
    heading.place(x=150, y=30, width = 250)

    productName_label = Label(passwordPage,text = "NEW PASSWORD",font=("yu gothic ui", 14, "bold"), bg= "white", fg = "#9d9d9d" )
    productName_label.place(x=40, y=95, width = 200)
    name = Entry(passwordPage, textvariable=PASSWORD,relief=FLAT, bg="white",fg= "black",  font=("yu gothic ui", 12, "bold"), width=15, show="*")
    name.place(x=65, y=130, width=200)

    productQt_label = Label(passwordPage,text = "CONFIRM PASSWORD",font=("yu gothic ui", 14, "bold"), bg= "white", fg = "#9d9d9d" )
    productQt_label.place(x=60, y=220, width = 200)
    qty = Entry(passwordPage, textvariable=CONFIRMPASSWORD,relief=FLAT, bg="white",fg= "black",  font=("yu gothic ui", 12, "bold"), width=15, show="*")
    qty.place(x=65, y=255, width=200)
...
def updatePassword():
    ...
    global npassword
    Database()
    cursor.execute("UPDATE admin SET password='{}' WHERE username='darshal' ".format(str(PASSWORD.get())))
    conn.commit()
    cursor.close()
    conn.close()
...
def Home1():
    ...
    transactionBtn.place(x=150, y=450)
...
def Home():
    ...
    newProductBtn.place(x=110, y=318)
    ...
    changePassBtn = Button(Title,image=changePassBtnImage, command=changePassword, relief=FLAT,bg="white",activebackground="white", borderwidth=0, cursor="hand2")
    changePassBtn.image = changePassBtnImage
    changePassBtn.place(x=110, y=500)

    logoutBtnImage = ImageTk.PhotoImage(file='images\logout.png')
    logoutBtn = Button(Title,image=logoutBtnImage, command=Logout, relief=FLAT,bg="white",activebackground="white", borderwidth=0, cursor="hand2")
    logoutBtn.image = logoutBtnImage
    logoutBtn.place(x=75, y=580)

    exitBtnImage = ImageTk.PhotoImage(file='images\exit.png')
    exitBtn = Button(Title,image=exitBtnImage, command=Exit, relief=FLAT,bg="white",activebackground="white", borderwidth=0, cursor="hand2")
    exitBtn.image = exitBtnImage
    exitBtn.place(x=250, y=580)
...
#window for updating product Page
def updateProductPage():
    if not tree.selection():
        tkMessageBox.showinfo("Warning!!!","Please Select A Product From The List")
    else:
        global updatePage
        updatePage = Toplevel()
...
#window for updating transaction page
def updateTransactionPage():
    if not tree2.selection():
        tkMessageBox.showinfo("Warning!!!","Please Select A Transaction From The List", icon="warning")
    else:
